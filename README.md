...
- Throughout the repository, the name `p4rl` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of p4rl (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/p4rl
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/p4rl/p4rl/ui_extension_example.py`.
